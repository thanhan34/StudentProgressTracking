...
export default function Dashboard() {
  ...
  const exportCsv = () => {
    const rows = [
      ["Mã", "Họ tên", "Giảng viên", "Trợ giảng", "Ngày bắt đầu", "Ngày thi dự kiến", "Giai đoạn", "Điểm hiện tại", "Mục tiêu", "Chuyên cần", "Task yếu", "Trạng thái"],
      ...filteredStudents.map((student) => [student.id, student.name, supportNames(student.instructors, "; "), supportNames(student.teachingAssistants, "; "), formatDate(student.startDate), formatDate(student.examDate), student.phase, student.currentScore, student.targetScore, `${student.attendance}%`, weakestTask(student)?.code ?? "Chưa có", student.status]),
    ];
...
function StudentDetail({ student, nextStudentName, onNext, onClose, onEdit, onDelete, onManageTasks, onWeeklyUpdate }: { student: Student; nextStudentName?: string; onNext?: () => void; onClose: () => void; onEdit: () => void; onDelete: () => void; onManageTasks: () => void; onWeeklyUpdate: (reportIndex?: number) => void }) {
  return <div className="modal-backdrop" onClick={onClose}><article className="student-modal pte-modal" onClick={(event) => event.stopPropagation()}>
    ...
    <header className="profile-header"><div className="modal-avatar" style={{ background: `${student.color}18`, color: student.color }}>{student.initials}</div><div><h2>{student.name}</h2><p style={{ overflowWrap: "anywhere" }}>{student.id}</p><span className={statusStyle[student.status]}><i />{student.status}</span></div><div className="exam-countdown"><small>{examCountdown(student.examDate)}</small></div></header>
